...
module "kubernetes" {
  ...
  additional_node_pools = contains(["ptlsbox", "ptl"], var.env) ? tolist([local.linux_node_pool]) : (contains(["sbox"], var.env) ? tolist([local.linux_node_pool, local.system_node_pool, local.arm_node_pool]) : tolist([local.linux_node_pool, local.system_node_pool, local.arm_node_pool]))
