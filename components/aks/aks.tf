...
module "kubernetes" {
  ...
  additional_node_pools = contains(["ptlsbox", "ptl", "sbox"], var.env) ? tolist([
    ...
    {
      name                = "msnode"
      vm_size             = lookup(coalesce(each.value.windows_node_pool, {}), "vm_size", "Standard_D4ds_v5")
      min_count           = lookup(coalesce(each.value.windows_node_pool, {}), "min_nodes", 2)
      max_count           = lookup(coalesce(each.value.windows_node_pool, {}), "max_nodes", 4)
      max_pods            = lookup(coalesce(each.value.windows_node_pool, {}), "max_pods", 30)
      os_type             = "Windows"
      os_sku              = lookup(coalesce(each.value.windows_node_pool, {}), "os_sku", "Windows2019")
